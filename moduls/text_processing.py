...
def prepare_summary_request(text: str) -> str:
    ...
    вернуть результат в виде корректного JSON-объекта с обязательными ключами:
        - "Type" (значение: "программист" или "проект")
    ...
    """
    prompt = (
        "Анализируй предоставленный текст. Текст может содержать одну из следующих форм:\n"
        "- резюме кандидата (собеседование или описание),\n"
        "- описание проекта.\n\n"
        "Твоя задача — извлечь информацию и вернуть строго JSON-объект с ровно следующими ключами (все ключи должны быть в нижнем регистре):\n"
        "  \"type\", \"name\", \"description\", \"stack\", \"skils\", \"telephone\", \"email\", \"telegram\".\n\n"
        "Правила заполнения:\n"
        "1. Поля \"stack\", \"skils\" и \"description\" не должны быть пустые. Бери все данные только из текста\n"
        "2. Поле \"name\":\n"
        ...
        "3. Поле \"type\" должно быть строго:\n"
        "   - \"project\", если текст описывает проект;\n"
        "   - \"kandidate\", если текст относится к кандидату (резюме или собеседование).\n"
        "4. Поля \"telephone\", \"email\" и \"telegram\" должны содержать предоставленные данные или значение \"отсутствует\", если данных нет.\n"
        "5. Возвращай только JSON без каких-либо пояснений или дополнительного текста.\n\n"
